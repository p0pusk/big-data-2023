...
def init_data_from_table(csv_path: str):
    # open csv file
    data = pd.read_csv(
        csv_path, sep=";", index_col=False, encoding="utf-8", comment="#"
    )
    data = pd.DataFrame(
        {"datetime": data["Местное время в Санкт-Петербурге"], "T": data["T"]}
    ).dropna()
    print(data)

    new_features = ["year", "mon", "day", "hour", "min"]

    for new_feature in new_features:
        data[new_feature] = data["datetime"].apply(
            lambda line: line_criterius(line)[new_feature]
        )
...
def init_polynome_regrestion_model(poly_degree: int, X: np.array, y: np.array):
    ...
    feature_selector = feature_selection.SequentialFeatureSelector(
        lr_model, k_features=32, forward=True
    )
...
def task_2(csv_path: str, poly_degree: int):
    ...
    output_linear_regrestion_errors(mlr.predict(X=X), y)
    plt.plot(mlr, X, label=f"Degree {poly_degree}")
    plt.show()
...
if __name__ == "__main__":
    ...
    ORIGIN_Y_DISTRIBUTION = (
        lambda x1, x2, x3, noise=None: 1 + 3 * x1 - 2 * x2 + x3 + noise
    )
