...
def huber(data: np.array, k: float):
    phi = lambda elem: elem if abs(elem) < k else k * np.sign(elem)

    return np.mean([phi(elem) for elem in data])
...
def double_stage_mean(x: np.array):
    ...
    filtered_data = x[(x >= lower_bound) & (x <= upper_bound)]
...
distributions = {
    ...
    "mix": lambda size: 0.9 * stats.norm.rvs(size=size) + 0.1 * stats.cauchy.rvs(
        size=size
    ),
}
